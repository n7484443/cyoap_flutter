...
class Tuple<A, B> {
  ...
  Tuple.fromJson(Map<String, dynamic> json)
      : data1 = json['data1'],
        data2 = json['typeB'] == 'ValueTypeVisible'
            ? ValueTypeWrapper.fromJson(json['data2'])
            : json['data2'];
  ...
  Tuple<double, double> operator *(num sizeMultiply) {
    if (data1 is num && data2 is num) {
      double alpha =
          data1 is double ? data1 as double : (data1 as int).toDouble();
      double beta =
          data2 is double ? data2 as double : (data2 as int).toDouble();
