...
class DistributeImp extends Distribute {
  ...
  Future<Uint8List> _readFileAsUint8(String path) async{
    ...
    if(path.endsWith(".json")){
      var output = await JsIsolatedWorker().run(functionName: '_readFileFromAjaxWithJson', arguments: path);
      return Uint8List.fromList(utf8.encode(output as String));
    }else{
      var output = await JsIsolatedWorker().run(functionName: '_readFileFromAjax', arguments: path);
